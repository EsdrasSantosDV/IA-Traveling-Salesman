...
def genetic_algorithm():
    ...
    for gen in range(num_generations):
        ...
        ax1.plot(x, best_array, label='Maior', linewidth=3)
        if elitism == 0:
            ax1.plot(x, avg_array, label='Média', linewidth=2)
            # ax1.plot(x, worst_array, label='Menor', linewidth=1)

        ax1.set_title("Ponto Máximo")
        ...
        var_generations.set(f'{gen + 1}º')

        root.update_idletasks()
        root.update()

    # plt.plot(x, best_array, label='Maior', linewidth=3)
    # plt.plot(x, avg_array, label='Média', linewidth=2   )
    # plt.plot(x, worst_array, label='Menor', linewidth=1)

    # plt.xlabel('Geração')
    # plt.ylabel('Fitness')
    # plt.title('Três Curvas')
